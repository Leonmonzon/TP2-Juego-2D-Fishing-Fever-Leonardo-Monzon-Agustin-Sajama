...
namespace Fishing_Fever
{
    ...
    public class Game1 : Game
    {
        ...
        private void ActualizarLogicaJuego(GameTime gameTime, MouseState mouseActual, bool click)
        {
            ...
            // Mantenemos el volumen actualizado por si cambió en la pausa
            MediaPlayer.Volume = NivelesVolumen[IndiceVolumen];
            ...
            if (barraPesca.EstaActiva())
            {
                ...
                if (barraPesca.PescaCompletada) { logicaPeces.MostrarPez(); pescador.Resetear(); anzuelo.Resetear(); }
            }
            else
            {
                pescador.Update(gameTime);
                anzuelo.Update(gameTime);
                if (anzuelo.Estado == EstadoAnzuelo.Listo && click) { pescador.Lanzar(); logicaPeces.EsconderPez(); }
                if (anzuelo.Estado == EstadoAnzuelo.Listo && pescador.AnimandoTiro && pescador.FrameActual >= 2)
                {
                    anzuelo.Lanzar(pescador.Posicion + new Vector2(80, 60), new Vector2(-300f, -300f));
